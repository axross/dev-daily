...
- Covered topics are enumerated as the single source of truth in the structured-data file [.agents/skills/tech-news-fetch/references/topics.jsonl](.agents/skills/tech-news-fetch/references/topics.jsonl); the schema, application rules, and adjustment procedure are documented in the companion file [covered-topics.md](.agents/skills/tech-news-fetch/references/covered-topics.md). Skills MUST link to those files rather than restating the list inline; updating the JSONL is the only place a topic addition / removal / rename needs to land for the rest of the pipeline to follow.
- Covered languages: **English** and **Japanese (日本語)**. The author reads both fluently; primary sources in either language are in scope on every run, and skills MUST attempt both unless the user explicitly scopes one. Japanese-language coverage is not an optional extra — a run that surfaces only English sources is a partial run and SHOULD be reported as such.
...
- [tech-news-fetch](.agents/skills/tech-news-fetch/SKILL.md) — apply when retrieving and storing recent tech articles (English or Japanese) within a publish-date range into the *Knowledge Base › Articles* Notion database. Covers time-range parsing (trailing windows, offset windows, absolute ranges, single dates) and the rule to ask the user when no range is given, the bilingual language-coverage mandate (English + Japanese on every run), discovery heuristics (format preference RSS/Atom > JSON Feed > sitemap > web search; the publisher blocklist at `references/publisher-blocklist.jsonl` as the only static gate; the 5-signal content-depth scorecard — with a Japanese character-count fallback — applied uniformly to every personal-blog candidate; publish-date verification when discovering via search; the per-run exploration mandate that surfaces at least one source not previously written to the Articles DB), and the write-call atomicity rule and per-topic + per-language run reporting. Owns the project's editorial scope at `references/topics.jsonl` (the canonical covered-topics data and its mapping to the Notion `Topics` controlled vocabulary), with schema/rules/procedure documented in the companion `references/covered-topics.md`. Database-level details (identity, schema, dedup contract, lifecycle, Topics vocabulary) defer to [notion-article-database](.agents/skills/notion-article-database/SKILL.md).
